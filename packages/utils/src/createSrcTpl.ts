import { ImgSrcTpl, ImgSrcTplFactory } from './types'

const defaultGlobalVars = {
  webp: false,
  ratio: window.devicePixelRatio || 1,
}
const defaultGlobalSrcTpl: ImgSrcTplFactory = () => [(attrs) => attrs.src, defaultGlobalVars]

export function createSrcTpl(globalSrcTpl: ImgSrcTplFactory = defaultGlobalSrcTpl): ImgSrcTpl {
 ...
 * @returns
 */
export const createSrcTplOfAliOss: ImgSrcTplFactory = (globalVars = defaultGlobalVars) => [
  ({ src, rect }) => {
 ...
 * @returns
 */
export const createSrcTplOfKSYunKS3: ImgSrcTplFactory = (globalVars = defaultGlobalVars) => [
  ({ src, rect }) => {
    const configs: string[] = []
    if (rect.width && rect.height) {
      const ratioW = Math.floor(globalVars.ratio * rect.width)
 ...
 * @returns
 */
export const createSrcTplOfQiniu: ImgSrcTplFactory = (globalVars = defaultGlobalVars) => [
  ({ src, rect }) => {
    const configs: string[] = []
    if (rect.width && rect.height) {
 ...
 * @returns
 */
export const createSrcTplOfTencent: ImgSrcTplFactory = (globalVars = defaultGlobalVars) => [
  ({ src, rect }) => {
    const configs: string[] = []
    if (rect.width && rect.height) {
      const w = Math.floor(globalVars.ratio * rect.width)
